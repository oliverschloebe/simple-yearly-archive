...
function get_simpleYearlyArchive($format, $excludeCat) {
	...
	foreach ($jahreMitBeitrag as $aktuellesJahr) {
		for ($aktuellerMonat = 1; $aktuellerMonat <= 12; $aktuellerMonat++) {
			...
			$monateMitBeitrag[$aktuellesJahr->year][$aktuellerMonat] = $wpdb->get_results("SELECT ID, post_date, post_title, post_excerpt FROM $wpdb->posts WHERE post_type = 'post' AND post_status = 'publish' AND year(post_date) = '$aktuellesJahr->year' ORDER BY post_date desc");
		}
	}
						...
						if ($post->post_date <= $now) {
							...
							if (!$match) {
								...
								if(get_option('sya_excerpt')==TRUE) {
									...
									$ausgabe .= '<br /><div style="padding-left:'.$indent.'px" class="robots-nocontent"><cite>' . strip_tags($excerpt) . '</cite></div>';
								}
								$ausgabe .= '</li>';
								$excerpt = '';
							}
	...
	<div class="wrap">
      ...
      <input type="hidden" name="action" value="edit" />
		...
		<ul>
			<li>
				<div class="left">
					<?php _e('Show optional Excerpt (if available)?', 'simple-yearly-archive'); ?>
				</div>
				<div>
		...
		<ul>
			<li>
				<div class="left">
					<div style="padding-left:20px;">-- <?php _e('Max. chars of Excerpt (0 for default)', 'simple-yearly-archive'); ?></div>
				</div>
				<div>
		...
		<ul>
			<li>
				<div class="left">
					<div style="padding-left:20px;">-- <?php _e('Indentation of Excerpt (in px)', 'simple-yearly-archive'); ?></div>
				</div>
				<div>
