		...
		foreach($jahreMitBeitrag as $aktuellesJahr) {
    		...
    		while ($aktuellerMonat >= 1) {
    				...
    				if ($monateMitBeitrag[$aktuellesJahr->year][$aktuellerMonat]) {
    					echo (''.$before.$linkyears_prepend.$aktuellesJahr->year.$linkyears_append.$after.'');
					echo '<ul>';
						...
						if ($post->post_date <= $now) {
							...
							if (!$match)
								echo ('<li>'.date(get_option('sya_dateformat'),strtotime($post->post_date)).' '.get_option('sya_datetitleseperator').' <a href="'.get_permalink($post->ID).'" title="'.$post->post_title.'">'.$post->post_title.'</a></li>');
						}
						}
    					echo '</ul>';
				}
...
function sya_inline($content) {
	...
	return str_replace('<!--simple-yearly-archive-->', simpleYearlyArchive(true), $content);
}
