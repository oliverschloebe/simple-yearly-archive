    		...
    		while ($aktuellerMonat >= 1) {
    				...
    				if ($monateMitBeitrag[$aktuellesJahr->year][$aktuellerMonat]) {
						...
						if ($post->post_date <= $now) {
							...
							if (!$match)
								$ausgabe .= ('<li>'.date(get_option('sya_dateformat'),strtotime($post->post_date)).' '.get_option('sya_datetitleseperator').' <a href="'.get_permalink($post->ID).'" title="'.$post->post_title.'">'.$post->post_title.'</a></li>');
						}
						}
